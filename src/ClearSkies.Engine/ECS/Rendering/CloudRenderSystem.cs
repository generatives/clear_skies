...
/// Depth-tested real geometry, so it belongs in <see cref="RenderPass.World"/> (after the terrain, which occludes
...
public sealed class CloudRenderSystem : IRenderSystem, IDisposable
{
    ...
    public CloudRenderSystem(Renderer renderer) => _clouds = new CloudLayer(renderer);
