...
/// <see cref="ModelRenderSystem"/> needs to draw it: a <see cref="RenderedModel"/> of the block type's shared model
/// (with its own pose, so it animates independently of every other block of its type) and <see cref="VoxelLit"/> to
/// light it from its own cell. Done here rather than when <see cref="ChunkVolume"/>
